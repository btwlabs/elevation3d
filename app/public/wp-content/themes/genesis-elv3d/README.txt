...
GitHub project link: https://github.com/studiopress/genesis-elv3d/.


## Installation Instructions

1. Upload the Genesis ELV3D theme folder via FTP to your wp-content/themes/ directory. (The Genesis parent theme needs to be in the wp-content/themes/ directory as well.)
2. Go to your WordPress dashboard and select Appearance.
3. Activate the Genesis ELV3D theme.
...
The version of [Genesis ELV3D on GitHub](https://github.com/studiopress/genesis-elv3d/) includes tooling to check code against WordPress standards. To use it:
...
2. In the command line, change directory to the Genesis ELV3D folder.
...
You'll see output highlighting issues with PHP files that do not conform to Genesis ELV3D coding standards.
...
- `npm run makepot` to regenerate the `languages/genesis-elv3d.pot` file.
...
- `npm run zip` to create a genesis-elv3d.zip. Files in the `excludes` array in `scripts/makezip.js` are omitted.
...
4. Type `npm run zip` to create `genesis-elv3d.zip`. Files in the `excludes` array in `scripts/makezip.js` are omitted from the zip. `filename.md` files will be renamed to `filename.txt`.
